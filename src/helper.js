...
module.exports = () => {
  ...
  template.registerHelper('h_github_corner', (data, options) => {
    const escape = data['repository'].replace(/git@|.git/g, '')
    const replace = escape.replace(/:/g, '/')
    let position = 'absolute'

    if (options['hash'] && options['hash']['fixed']) {
      position = 'fixed'
    }
    return `
      <a href="https://${replace}" class="github-corner" aria-label="View source on GitHub">
      ...
      <style>.github-corner{position:${position};top:0;right:0;}.github-corner:hover.octo-arm{animation:octocat-wave560msease-in-out}@keyframesoctocat-wave{0%,100%{transform:rotate(0)}20%,60%{transform:rotate(-25deg)}40%,80%{transform:rotate(10deg)}}@media (max-width:500px){.github-corner:hover .octo-arm{animation:none}.github-corner .octo-arm{animation:octocat-wave 560ms ease-in-out}}</style>
    `
  })
}